...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 02, 2022 at 09:00 AM
-- Server version: 10.4.22-MariaDB
-- PHP Version: 8.1.2

CREATE DATABASE IF NOT EXISTS Ordzilla DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;
USE Ordzilla;
...
(1, 'root', '$2y$10$yshs5uC2CYOfCJ.CjZ3rCOqQ3nrfUVSfaRJO3RMH55qe1RzJWxFku');
...
(5, 'Cooler Master B120i CPU Water-Cooled 120mm', 3000, 23, 2, 4, 112, '927a7da5b8db838d6e71923050b13d9a88643944.jpg'),
...
(4, 'i9-11900K - 11th Gen Rocket Lake 8-Core ', 7, 3, 1, 5, 749, '06830cb0b58634d5daf56a8584b62c5b384ac83f.jpg'),
(6, 'Intel Core i5-11600 - 11th Gen Rocket Lake 6-Core ', 7, 2, 1, 2, 349, '401b63dae346d597fc2b850e9813022b86b78032.jpg'),
(7, 'Intel Core i5-10400 - 10th Gen Comet Lake 6-Core', 6, 2, 1, 11, 194, '097770b611bf1e490ca743a561643b3f36407414.jpg'),
...
(2, ' RTX 3080 FTW3 ULTRA GAMING Video Card', 450, 4, 11, 1249, '4e81fba6e541f98f0c3c187ed45b396f6d491b89.jpg'),
...
(25, 'ZOTAC GAMING GeForce GTX 1650 OC 4GB GDDR6', 200, 9, 2, 229, '87ee41d42819310e61d29eb794ec2a584c254413.jpg'),
...
(13, 'Fractal Design Ion Gold 550W 80 PLUS Gold', 550, 33, 1, 4, 109, 'b2d3056464b06ddf9a74f67acd96d1e244914f06.jpg'),
...
(24, ' G.SKILL Trident Z Royal Elite Series 64GB (4 x 16GB) ', 1, 3600, 12, 2, 588, '0f1ab4e57e4cb58bdd51c5ac22bfd1781fe3941b.jpg'),
...
(28, 'PNY 16GB 288-Pin PC RAM DDR4 3200 ', 1, 3200, 18, 4, 89, '85dda5db06dcec563760ca2f20162ddc9962c25f.jpg'),
...
CREATE VIEW allpieceinstock AS
SELECT "cooling" AS "typePiece",
    cooling.coo_id AS "id",
    cooling.coo_name AS "nom",
    company.com_name AS "company",
    cooling.coo_clock AS "vitesseWatt",
    cooling.coo_watercooling AS "type",
    null AS "typeCapacity",
    cooling.coo_amount AS "quantite",
    cooling.coo_prix AS "prix",
    cooling.coo_path as 'path'
FROM cooling
    JOIN company ON company.com_id = cooling.coo_company
WHERE cooling.coo_amount > 0
UNION
SELECT "cpu" AS "typePiece",
    cpu.cpu_id AS "id",
    cpu.cpu_name AS "nom",
    company.com_name AS "company",
    cpu.cpu_speed AS "vitesseWatt",
    type_cpu.tyc_type AS "type",
    null AS "typeCapacite",
    cpu.cpu_amount AS "quantite",
    cpu.cpu_prix AS "prix",
    cpu.cpu_path as 'path'
FROM cpu
    JOIN company ON company.com_id = cpu.cpu_company
    JOIN type_cpu ON type_cpu.tyc_id = cpu.cpu_type
WHERE cpu.cpu_amount > 0
UNION
SELECT "gpu" AS "typePiece",
    gpu.gpu_id AS "id",
    gpu.gpu_name AS "nom",
    company.com_name AS "company",
    gpu.gpu_watt AS "vitesseWatt",
    null AS "type",
    null AS "typeCapacite",
    gpu.gpu_amount AS "quantite",
    gpu.gpu_prix AS "prix",
    gpu.gpu_path as 'path'
FROM gpu
    JOIN company ON company.com_id = gpu.gpu_company
WHERE gpu.gpu_amount > 0
UNION
SELECT "motherboard" AS "typePiece",
    motherboard.mbo_id AS "id",
    motherboard.mbo_name AS "nom",
    company.com_name AS "company",
    null AS "vitesseWatt",
    type_motherboard.tym_type AS "type",
    null AS "typeCapacite",
    motherboard.mbo_amount AS "quantite",
    motherboard.mbo_prix AS "prix",
    motherboard.mbo_path as 'path'
FROM motherboard
    JOIN company ON company.com_id = motherboard.mbo_company
    JOIN type_motherboard ON type_motherboard.tym_id = motherboard.mbo_type
WHERE motherboard.mbo_amount > 0
UNION
SELECT "pc_case" AS "typePiece",
    pc_case.pca_id AS "id",
    pc_case.pca_name AS "nom",
    company.com_name AS "company",
    null AS "vitesseWatt",
    type_motherboard.tym_type "type",
    null AS "typeCapacite",
    pc_case.pca_amount AS "quantite",
    pc_case.pca_prix AS "prix",
    pc_case.pca_path as 'path'
FROM pc_case
    JOIN company ON company.com_id = pc_case.pca_company
    JOIN type_motherboard ON type_motherboard.tym_id = pc_case.pca_type_mb_id
WHERE pc_case.pca_amount > 0
UNION
SELECT "psu" AS "typePiece",
    psu.psu_id AS "id",
    psu.psu_name AS "nom",
    company.com_name AS "company",
    psu.psu_watt AS "vitesseWatt",
    type_psu.typ_type "type",
    null AS "typeCapacite",
    psu.psu_amount AS "quantite",
    psu.psu_prix AS "prix",
    psu.psu_path as 'path'
FROM psu
    JOIN company ON company.com_id = psu.psu_company
    JOIN type_psu ON type_psu.typ_id = psu.psu_type
WHERE psu.psu_amount > 0
UNION
SELECT "ram" AS "typePiece",
    ram.ram_id AS "id",
    ram.ram_name AS "nom",
    company.com_name AS "company",
    ram.ram_speed AS "vitesseWatt",
    type_ram.tyr_type AS "type",
    null AS "typeCapacite",
    ram.ram_amount AS "quantite",
    ram.ram_prix AS "prix",
    ram.ram_path as 'path'
FROM ram
    JOIN company ON company.com_id = ram.ram_company
    JOIN type_ram ON type_ram.tyr_id = ram.ram_type_ram_id
WHERE ram.ram_amount > 0
UNION
SELECT "storage" AS "typePiece",
    storage.sto_id AS "id",
    storage.sto_name AS "nom",
    company.com_name AS "company",
    storage.sto_capacity AS "vitesseWatt",
    type_storage.tys_type "type",
    type_capacity.tca_type AS "typeCapacite",
    storage.sto_amount AS "quantite",
    storage.sto_prix AS "prix",
    storage.sto_path as 'path'
FROM storage
    JOIN company ON company.com_id = storage.sto_company
    JOIN type_storage ON type_storage.tys_id = storage.sto_type_storage_id
    JOIN type_capacity ON type_capacity.tca_id = storage.sto_type_capacity
WHERE storage.sto_amount > 0;




CREATE VIEW allpiece AS
SELECT "cooling" AS "typePiece",
    cooling.coo_id AS "id",
    cooling.coo_name AS "nom",
    company.com_name AS "company",
...
--
ALTER TABLE `gpu`
  MODIFY `gpu_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=28;
...
--
ALTER TABLE `motherboard`
  MODIFY `mbo_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=26;
...
--
ALTER TABLE `type_motherboard`
  MODIFY `tym_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
